...
public class Restaurant_EnvoyerMessage extends JFrame {
	String User=DashBord.User_now;
	
	private static int ID_Message=NbrRowTable()+1;
	 ...
	 * Create the frame.
	 */
	public Restaurant_EnvoyerMessage() {
				...
				if(User.equals("")) {
					...
					if(Destinataire.equals("")||Message.equals("")) {
						...
						try {	
							...
							while(rs.next()) {
								if(rs.getString("Username").equals(Destinataire)&&Destinataire.indexOf("distribution")!= -1) {
									verify=1;
								}
							}
							rs.close();
