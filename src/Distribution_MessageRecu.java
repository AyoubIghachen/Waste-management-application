...
public class Distribution_MessageRecu extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Distribution_MessageRecu frame = new Distribution_MessageRecu();
	 ...
	 * Create the frame.
	 */
	public Distribution_MessageRecu() {
		...
		try {	
            ...
            String sql ="SELECT ID,User,Message FROM Message WHERE Destinataire = ?";
			...
			if(BigData!=null) {
				table.addMouseListener(new MouseAdapter() {
					...
					//	String From = model.getValueAt(selectedRowIndex, 1).toString();
						...
						Message_From frameMessage = new Message_From(BigData[selectedRowIndex]);
		...
		lblReceivedMessage = new JLabel("Received Messages");
